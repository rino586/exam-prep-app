...
def init_db():
    ...
    """)
    # 既存DBに steps/figure カラムがない場合は追加
    try:
        conn.execute("ALTER TABLE generated_problems ADD COLUMN steps TEXT NOT NULL DEFAULT ''")
    except Exception:
        pass
    try:
        conn.execute("ALTER TABLE generated_problems ADD COLUMN figure TEXT NOT NULL DEFAULT ''")
    except Exception:
        pass
    conn.commit()
    conn.close()
  ...
  "problems": [
    {
      ...
      "hint": "考え方のヒント",
      "figure": "図や表が役立つ場合はテキストで記載（不要なら空文字）"
    }
  ]
}"""
...
【figureフィールドの書き方】
- 語の構成・対義語・文の構造などで図解できる場合はテキストで記載
- 例（語の構成）：「春風　→　春＋風（季節＋自然）」
- 不要な場合は空文字にする"""
...
def save_generated(subject: str, original_label: str, generated: list) -> int:
    ...
    for p in generated:
        conn.execute(
            "INSERT INTO generated_problems (original_problem_id, problem_text, answer, steps, hint, figure, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (problem_id, p["problem"], p["answer"], p.get("steps", ""), p["hint"], p.get("figure", ""), datetime.now().strftime("%Y-%m-%d %H:%M"))
        )
    conn.commit()
    conn.close()
    return problem_id
